...
const CORES_T = { RotaMax:'#818cf8', ViaCargo:'#38bdf8', FlashLog:'#fb7185' };
const CORES_R = {
  Sudeste:'#a78bfa', Sul:'#34d399', Nordeste:'#f59e0b',
  Norte:'#60a5fa', 'Centro-Oeste':'#f472b6'
};

function status(d) { return d.real > d.prazo ? 'atrasado' : 'no-prazo'; }
function excesso(d) { return Math.max(0, d.real - d.prazo); }
...
function renderKPIs(dados) {
  ...
  document.getElementById('kpi-pct-atraso').textContent =
    total ? Math.round(atrasadas.length/total*100)+'% do total' : '—';
  document.getElementById('kpi-pct-prazo').textContent =
    total ? Math.round(noPrazo.length/total*100)+'% do total' : '—';
}
...
function renderBarras(dados) {
  ...
  const totalByT = {};
  dados.forEach(d => {
    if (!byT[d.transportadora]) { byT[d.transportadora]=0; totalByT[d.transportadora]=0; }
  ...
  elT.innerHTML = Object.entries(totalByT).sort((a,b)=>b[1]-a[1]).map(([name, tot]) => {
  ...
  const totalByR = {};
  dados.forEach(d => {
    if (!byR[d.regiao]) { byR[d.regiao]=0; totalByR[d.regiao]=0; }
    totalByR[d.regiao]++;
  ...
  elR.innerHTML = Object.entries(totalByR).sort((a,b)=>b[1]-a[1]).map(([name, tot]) => {
...
function renderRanking(dados) {
  ...
  if (!atrasadas.length) {
    el.innerHTML = '<div style="color:var(--muted);font-size:13px;">Nenhuma entrega atrasada na seleção.</div>';
    return;
  }
  el.innerHTML = atrasadas.map((d, i) => {
...
function renderTabela(dados) {
  ...
  tbody.innerHTML = sorted.map(d => {
    ...
    const badgeClass = st==='atrasado' ? (ex>=5?'critico':'atrasado') : 'no-prazo';
    const badgeLabel = st==='atrasado' ? (ex>=5?'⚠ Crítico':'Atrasado') : '✓ No Prazo';
    return `<tr>
...
function renderAlerta(dados) {
  ...
  // pior transportadora
  const byT = {};
  atrasadas.forEach(d => { byT[d.transportadora]=(byT[d.transportadora]||0)+1; });
  const piorT = Object.entries(byT).sort((a,b)=>b[1]-a[1])[0];
  if (piorT) msg += ` Transportadora com mais atrasos: <strong>${piorT[0]}</strong> (${piorT[1]}).`;
